...
const ChatWindow = () => {
  ...
  const [messages, setMessages] = useState([{ text: 'Hey there! 👋', sender: 'bot' }]);
  ...
  return (
    <div className="w-[700px] h-[500px] bg-white/10 backdrop-blur-md border border-white/30 rounded-3xl shadow-2xl text-white flex flex-col overflow-hidden relative">
      ...
      {/* Emoji Picker */}
      {showEmojiPicker && (
        <div className="absolute bottom-24 left-6 z-50 w-72 bg-white/20 backdrop-blur-md p-3 rounded-xl shadow-lg">
          {/* Tabs */}
            ...
            {Object.keys(emojiCategories).map((cat) => (
              <button
                ...
                className={`text-2xl px-2 py-1 rounded-full transition ${
                  activeTab === cat ? 'bg-white/30 scale-110' : 'hover:bg-white/10'
                }`}
              >
                {cat}
              </button>
